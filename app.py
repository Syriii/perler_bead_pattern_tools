...
# 兼容旧键名：global_replacements（如果存在且新键为空，则迁移）
if 'global_replacements' in st.session_state:
    try:
        if st.session_state.global_replacements and not st.session_state.color_replacements:
            st.session_state.color_replacements = dict(st.session_state.global_replacements)
    finally:
...
def make_swatch(hex_color: str, width: int = 60, height: int = 24) -> Image.Image:
    """创建用于展示的颜色色块图像"""
    try:
        return Image.new('RGB', (width, height), normalize_hex(hex_color))
    except Exception:
        # 回退到白色避免渲染错误
    ...
    with st.expander("颜色替换"):
                ...
                if old_color_name:
                    ...
                    if hex_old:
                        st.markdown(
                            ...
                            f"<span style='display:inline-block;width:24px;height:24px;border:1px solid #ccc;background-color:{normalize_hex(hex_old)}'></span>"
            ...
            with col_new:
                ...
                if new_color_name:
                    ...
                    if hex_new:
                        st.markdown(
                            f"<div style='display:flex;align-items:center;gap:8px;margin:6px 0'>"
                            f"<span style='display:inline-block;width:24px;height:24px;border:1px solid #ccc;background-color:{normalize_hex(hex_new)}'></span>"
        ...
        if st.session_state.color_replacements:
            ...
            for k, v in st.session_state.color_replacements.items():
                hex_old = normalize_hex(palette_colors_main.get(k, '#FFFFFF'))
                hex_new = normalize_hex(palette_colors_main.get(v, '#FFFFFF'))
                cell_old = (
...
def main():
    ...
    # 多页面应用：本页为“生成图案”，侧边栏由 Streamlit 自动生成页面导航
    # 仅渲染主处理功能
    
    # 生成图案模块
    
    # 侧边栏：拼豆图案设置（与全局设置区分，置于下方）
    with st.sidebar:
        st.divider()
        st.header("拼豆图案设置")
    ...
    if st.session_state.processor is not None:
            ...
            if color_stats:
                # 创建颜色统计表格
                # 使用自定义 HTML 表格，颜色列同排显示色块与拼豆编码
                palette_colors = get_palette_colors(selected_palette)
                total = sum(color_stats.values())
                stats_rows = []
                for color_name, count in list(color_stats.items())[:10]:  # 显示前10种颜色
                    if color_name in palette_colors:
                        hex_color = normalize_hex(palette_colors[color_name])
                        cell_color = (
                            f"<div style='display:flex;align-items:center;gap:8px'>"
